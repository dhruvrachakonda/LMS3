...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        string CreateNewUser(string firstName, string lastName, DateTime DOB, string departmentAbbrev, string role)
        {
            ...
            //We will generate a new UID
            string newIDFull = "";

            //If we start with no users, then we start with 0000000000
            if(!db.Administrators.Any() && !db.Professors.Any() && !db.Students.Any())
            {
                newIDFull = "u0000000";
            }

            //else we combine all the uids existing into one query and find the largest of them all 
            else {
                var allUIds = db.Professors.Select(prof => prof.UId)
                .Concat(db.Students.Select(student => student.UId))
                .Concat(db.Administrators.Select(admin => admin.UId));

                var nextID = allUIds.Max();

                int newID = int.Parse(nextID.Substring(1));

                //we then add 1 to it
                newID++;

                //and that is our new unid 
                newIDFull = "u" + newID.ToString();
            }

            //we then add it using LINQ to the appropriate table
